...
For demo step capabilities (`@executioncontrolprotocol/demo.echo`, etc.), use `registerDemoExtension()` from `@executioncontrolprotocol/demo` (not exported from core).
...
| Path | Status |
| ---- | ------ |
| `examples/01-echo/` | Runnable (`@executioncontrolprotocol/demo.echo`) |
