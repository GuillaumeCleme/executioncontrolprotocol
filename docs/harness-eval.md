...
Intent eval environments bind `@executioncontrolprotocol/format-toon` and `@executioncontrolprotocol/demo` (same as workflow) and include a **summarized** environment capability block in the user prompt (see `_internal/summarize-environment` in `@executioncontrolprotocol/evals`). System prompts come from [`packages/core/fixtures/harness-prompts/`](../packages/core/fixtures/harness-prompts/) via `buildSystemPrompt()` — not from eval case JSON.
