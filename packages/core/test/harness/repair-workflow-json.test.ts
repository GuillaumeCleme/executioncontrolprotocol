...
describe("repairWorkflowJsonSyntax", () => {
  it("returns valid JSON unchanged (still parseable)", () => {
    const valid = JSON.stringify({
      ...
      version: "1.0",
      workflow: { id: "w" },
      steps: [{ type: "step", id: "a", uses: "@executioncontrolprotocol/demo.echo", input: {}, as: "a" }],
    })
  ...
  it("moves a floating 'as' back inside the preceding step", () => {
    const raw =
      '{"schema":"@executioncontrolprotocol.workflow","version":"1.0","workflow":{"id":"w"},"steps":[{"type":"step","id":"echo","uses":"@executioncontrolprotocol/demo.echo","input":{"value":"hi"}},"as":"echo"},{"type":"step","id":"two","uses":"@executioncontrolprotocol/demo.echo","input":{}}]}'
    const repaired = repairWorkflowJsonSyntax(raw)
  ...
  it("preserves $ref inputs with legitimate nested braces", () => {
    const raw = JSON.stringify({
      ...
      workflow: { id: "w" },
      steps: [
        { type: "step", id: "a", uses: "@executioncontrolprotocol/demo.echo", input: { x: { $ref: "state.y" } }, as: "a" },
      ],
    })
    const repaired = repairWorkflowJsonSyntax(raw)
