...
describe("applyPatch", () => {
  ...
  it("replaces the steps array when path is steps", () => {
    const manifest = workflow("Echo")
      .id("echo-test")
      .run([step("@executioncontrolprotocol/demo.echo", "Echo").id("echo").with({ value: "hi" }).as("echo")])
      .toManifest()
  ...
  it("removes a step via eql:delete", () => {
    const manifest = workflow("Echo notify")
      .id("echo-notify")
      .run([
        step("@executioncontrolprotocol/demo.echo", "Echo").id("echo").with({ value: "hi" }).as("echo"),
  ...
  it("replaces scalar step fields without corrupting the step", () => {
    ...
    expect(echoStep?.uses).toBe("@executioncontrolprotocol/demo.echo")
  })
  ...
  it("fails when duplicate step ids exist", () => {
    const duplicate: WorkflowManifest = {
      ...
      workflow: { id: "dup" },
      steps: [
        { type: "step", id: "write-brief", uses: "@executioncontrolprotocol/demo.echo", input: {} },
        { type: "step", id: "write-brief", uses: "@executioncontrolprotocol/demo.echo", input: {} },
      ],
    }
  ...
  it("resolves nested parallel step paths", () => {
    const manifest = workflow("Nested")
      .run([
        parallel([
          [step("@executioncontrolprotocol/demo.echo", "Inner").with({ value: "a" }).as("inner")],
        ]),
      ])
      .toManifest()
...
describe("assignUniqueStepIds via toManifest", () => {
  it("assigns unique ids for duplicate labels", () => {
    const manifest = workflow("Dup labels")
      .run([
        step("@executioncontrolprotocol/demo.echo", "Echo").with({ value: "a" }).as("o"),
        step("@executioncontrolprotocol/demo.echo", "Echo").with({ value: "b" }).as("o2"),
      ])
      .toManifest()
