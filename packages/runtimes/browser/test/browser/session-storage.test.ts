...
describe("@executioncontextprotocol/browser-session-config in browser", () => {
  it("reads values from localStorage via session config", async () => {
    ...
    expect(desc.extensions.some((e) => e.id === "@executioncontextprotocol/browser-session-config")).toBe(true)
