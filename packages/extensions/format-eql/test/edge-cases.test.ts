...
describe("EQL edge cases and coverage", () => {
  it("infers sourceSchema from document when omitted", () => {
    const manifest = {
      ...
      version: "1.0",
      workflow: { id: "x" },
      steps: [{ type: "step", id: "s", uses: "@executioncontrolprotocol/demo.echo" }],
    } as WorkflowManifest
...
DELETE STEP old
ADD STEP new USES @executioncontrolprotocol/demo.echo AFTER echo
  WITH value = "added"
  LABEL "New"
...
WORKFLOW bad
STEP s USES @executioncontrolprotocol/demo.echo
  WITH data = {not-json}`
  ...
  it("encodes UPDATE USES and skips non-step workflow nodes", () => {
    ...
    const manifest = {
      ...
      steps: [
        ...
        { type: "step", id: "s", uses: "@executioncontrolprotocol/demo.echo" },
      ],
    } as WorkflowManifest
