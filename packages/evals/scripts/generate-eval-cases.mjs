...
const matrixExt = ["@executioncontrolprotocol/format-toon", "@executioncontrolprotocol/format-eql", "@executioncontrolprotocol/format-json", "@executioncontrolprotocol/demo", "@executioncontrolprotocol/demo"]
const offTopicJudge = {
  enabled: true,
...
const creates = [
  ["wf-create-01", "Minimal echo", "Create a minimal @executioncontrolprotocol.workflow with one step id echo using @executioncontrolprotocol/demo.echo and input value hello.", [{ kind: "stepUses", capabilityId: "@executioncontrolprotocol/demo.echo" }, { kind: "stepCount", exact: 1 }]],
  ["wf-create-02", "Echo plus summarize", "Create a workflow with echo (@executioncontrolprotocol/demo.echo) then summarize (@executioncontrolprotocol/demo.summarize) passing echo output.", [{ kind: "stepCount", min: 2 }, { kind: "stepUses", capabilityId: "@executioncontrolprotocol/demo.summarize" }]],
  ["wf-create-03", "Validate then echo", "Build a workflow: first @executioncontrolprotocol/demo.validate then @executioncontrolprotocol/demo.echo.", [{ kind: "stepUses", capabilityId: "@executioncontrolprotocol/demo.validate" }]],
  ["wf-create-04", "Notify step", "Create workflow with @executioncontrolprotocol/demo.echo and a final @executioncontrolprotocol/demo.notify step.", [{ kind: "stepUses", capabilityId: "@executioncontrolprotocol/demo.notify" }]],
  ["wf-create-05", "Translate branch", "Create a two-step workflow using @executioncontrolprotocol/demo.echo and @executioncontrolprotocol/demo.translate.", [{ kind: "stepUses", capabilityId: "@executioncontrolprotocol/demo.translate" }]],
  ["wf-create-06", "Spanish label", "Crea un flujo con un paso echo usando @executioncontrolprotocol/demo.echo.", [{ kind: "stepUses", capabilityId: "@executioncontrolprotocol/demo.echo" }]],
  ["wf-create-07", "French label", "Créez un workflow avec une étape @executioncontrolprotocol/demo.echo.", [{ kind: "stepUses", capabilityId: "@executioncontrolprotocol/demo.echo" }]],
  ["wf-create-08", "German label", "Erstelle einen Workflow mit @executioncontrolprotocol/demo.echo.", [{ kind: "stepUses", capabilityId: "@executioncontrolprotocol/demo.echo" }]],
  ["wf-create-09", "Triple steps", "Create a 3-step workflow using @executioncontrolprotocol/demo.validate, @executioncontrolprotocol/demo.echo, and @executioncontrolprotocol/demo.summarize.", [{ kind: "stepCount", min: 3 }]],
  ["wf-create-10", "Workflow id minimal-echo", "Create workflow id minimal-echo with one @executioncontrolprotocol/demo.echo step labeled Runner.", [{ kind: "stepUses", capabilityId: "@executioncontrolprotocol/demo.echo" }]],
  ["wf-create-11", "Quality judge", "Design a clear production workflow using @executioncontrolprotocol/demo.echo for ingestion.", [], { enabled: true, goal: "Workflow is coherent and references echo capability", requireApproved: true }],
  ["wf-create-12", "Descriptor caps", "List capabilities then create echo-only workflow with @executioncontrolprotocol/demo.echo.", [{ kind: "descriptorListsCapabilities", ids: ["@executioncontrolprotocol/demo.echo", "@executioncontrolprotocol/demo.summarize"] }]],
...
const assistants = [
  ...
  ["asst-10", "Capabilities list", "List supported step capabilities.", null, [{ kind: "answerContains", text: "demo.echo" }], true],
  ...
  ["asst-14", "Environment help", "What capabilities are available?", null, [{ kind: "answerContains", text: "demo.echo" }], { enabled: true, goal: "Lists capabilities", rubric: "Names capability ids from the environment", requireApproved: true }],
...
const flows = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        harness: "workflow-authoring",
        input: { request: "Create minimal @executioncontrolprotocol/demo.echo workflow." },
        assertions: { deterministic: [{ kind: "stepUses", capabilityId: "@executioncontrolprotocol/demo.echo" }], judge: { enabled: false } },
      },
    ],
  },
  {
