import { collectModelOutputFeedback, type CompactEnvironmentSummary } from "@ecp/core"
import {
  inferPatchTargetStepId,
  inferRequiredCapabilityIds,
} from "@ecp/harnesses-browser-nano"
import type { HarnessOperationFeedback, StepNode, WorkflowManifest } from "@ecp/types"
...
export function buildFluentPatchHintLines(
  ...
  const lines = [
    ...
    "Return a complete revised module: import { workflow, step, ref } from \"@ecp/core\" when using ref().",
    "Edit rules:",
  ...
  if (moveMatch) {
    ...
    if (required.length > 0) {
      lines.push(
        `Append step("${required[0]}", ...) to .run([...])${addAfterMatch ? ` after the step with id "${addAfterMatch[1]}"` : ""}; import ref from "@ecp/core" when chaining outputs.`
      )
    }
  }
...
export function collectFluentCompileErrorFeedback(
  ...
  if (
    ...
    return [
      collectModelOutputFeedback(
        "Use only @ecp/core Fluent API: workflow, step, ref, branch, parallel, loop. " +
