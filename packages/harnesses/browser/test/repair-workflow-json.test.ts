...
describe("repairWorkflowJsonSyntax", () => {
  it("returns valid JSON unchanged (still parseable)", () => {
    const valid = JSON.stringify({
      ...
      version: "1.0",
      workflow: { id: "w" },
      steps: [{ type: "step", id: "a", uses: "@ecp/test.echo", input: {}, as: "a" }],
    })
  ...
  it("moves a floating 'as' back inside the preceding step", () => {
    const raw =
      '{"schema":"@ecp.workflow","version":"1.0","workflow":{"id":"w"},"steps":[{"type":"step","id":"echo","uses":"@ecp/test.echo","input":{"value":"hi"}},"as":"echo"},{"type":"step","id":"two","uses":"@ecp/test.echo","input":{}}]}'
    const repaired = repairWorkflowJsonSyntax(raw)
  ...
  it("preserves $ref inputs with legitimate nested braces", () => {
    const raw = JSON.stringify({
      ...
      workflow: { id: "w" },
      steps: [
        { type: "step", id: "a", uses: "@ecp/test.echo", input: { x: { $ref: "state.y" } }, as: "a" },
      ],
    })
    const repaired = repairWorkflowJsonSyntax(raw)
