...
export function inferRequiredCapabilityIds(
  ...
  if (/\btranslate\b/i.test(request) && /\btranslate\b.*\bstep\b|\badd\b.*\btranslate\b|@ecp\/demo\.translate/i.test(request)) {
...
export function buildRequestCapabilityHintLines(
  ...
  if (stepIdMatch && mode === "create") {
    lines.push(
      `Step id must be "${stepIdMatch[1]}" (short name, not a capability id). Format: STEP ${stepIdMatch[1]} USES @ecp/...`,
      ""
    )
  }
