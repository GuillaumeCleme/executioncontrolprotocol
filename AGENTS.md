...
Extension packages call `catalogExtension(def)` at module load. **Examples use string bindings:** `extension("@executioncontrolprotocol/format-toon").with({})` after `import "@executioncontrolprotocol/format-toon"` (catalog lookup). For echo workflows, `import "@executioncontrolprotocol/demo"` then `extension("@executioncontrolprotocol/demo").with({})`.
